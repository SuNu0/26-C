...
class Parameters:
    ...
    throughput_cost_yuan_per_kwh: float = 0.005
...
def _dispatch_lp(
    ...
    """模型调用：用 HiGHS 线性规划求购电、充放电、剩余电量和 SOC。"""
    n = len(load_hat)
    if not (len(pv_hat) == len(price) == n and n > 0):
        raise ValueError("优化输入长度必须一致且非空")

    adjusted = base_grid is not None
    # 日前变量：[grid, charge, discharge, spill, soc]
    # 调整变量：[up, down, charge, discharge, spill, soc]
    blocks = 6 if adjusted else 5
    m = blocks * n
    idx = {
        "first": slice(0, n),
        "second": slice(n, 2 * n),
        ...
        "soc": slice((5 if adjusted else 4) * n, blocks * n),
    }

    c = np.zeros(m)
    bounds: list[tuple[float, float | None]] = [(0.0, None)] * m
    max_e = p.max_interval_energy_kwh
    grid_cap = np.maximum(load_hat + max_e, max_e)
    if adjusted:
        assert base_grid is not None
        c[idx["first"]] = p.up_adjust_multiplier * price
        c[idx["second"]] = -p.down_cancel_refund_multiplier * price
        for t in range(n):
            bounds[idx["first"].start + t] = (0.0, float(grid_cap[t]))
            bounds[idx["second"].start + t] = (0.0, float(max(base_grid[t], 0.0)))
    else:
        c[idx["first"]] = price
        for t in range(n):
            bounds[idx["first"].start + t] = (0.0, float(grid_cap[t]))

    c[idx["charge"]] = p.throughput_cost_yuan_per_kwh
    c[idx["discharge"]] = p.throughput_cost_yuan_per_kwh
    for t in range(n):
        bounds[idx["charge"].start + t] = (0.0, max_e)
        bounds[idx["discharge"].start + t] = (0.0, max_e)
        # 剩余量最多等于预测光伏，禁止通过“丢弃已购电”虚假获利。
        bounds[idx["spill"].start + t] = (0.0, float(max(pv_hat[t], 0.0)))
        bounds[idx["soc"].start + t] = (p.soc_min_kwh, p.soc_max_kwh)
    ...
    result = linprog(c, A_eq=a_eq.tocsr(), b_eq=b_eq, bounds=bounds, method="highs")
...
def solve_day_ahead(
    ...
    for day_i in range(stop_day):
        ...
        plan = _dispatch_lp(load_hat, pv_hat, price, soc, soc, p)
...
def solve_rolling(
    ...
    for day_i in range(stop_day):
        ...
        plan0 = _dispatch_lp(load0, pv0, price, soc, soc, p)
        q0 = plan0.grid.copy()
        q_final = q0.copy()
        ...
        for k, start in enumerate(issue_ts):
            ...
            if start == 0:
                ...
                current_plan = _dispatch_lp(
                    load_hat, pv_hat, price[start:], soc, day_start_soc, p, base_grid=q0[start:]
                )
